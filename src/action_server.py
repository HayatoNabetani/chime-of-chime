...
class ActionController:
    ...
    def handle(self, event: dict[str, Any]) -> None:
        if event.get("type") != "postback":
            return
        if event.get("source", {}).get("userId") != self.allowed_user_id:
        ...
        try:
            with self._lock:
                if action == "action=intercom_toggle":
        ...
        if isinstance(reply_token, str):
            self._reply(reply_token, result)

    def _reply(self, reply_token: str, message: str) -> None:
        try:
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self) -> None:  # noqa: N802
            ...
            for event in payload.get("events", []):
                threading.Thread(target=controller.handle, args=(event,), daemon=True).start()
...
def serve_from_env() -> None:
    ...
    if server is None:
        raise SystemExit(1)
    print(f"🎛️ 操作サーバー起動: http://{server.server_address[0]}:{server.server_address[1]}/line/webhook")
    try:
...
def start_in_background_from_env() -> ThreadingHTTPServer | None:
    ...
    if os.getenv("ACTION_SERVER_ENABLED", "1") != "1":
        return None
    targets = {part.strip().lower() for part in os.getenv("NOTIFIER", "console").split(",")}
